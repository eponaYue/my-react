...
window.vxgplayer = function(id, options) {
  ...
  if (!window.vxgplayer.players[id]) {
    if (typeof chrome == 'undefined') {
      ...
      var width = 640;
      var height = 480;
      width = parseInt(player.offsetWidth, 10) || width;
      height = parseInt(player.offsetHeight, 10) || height;
      ...
      var html =
        ...
        ' 请使用 <a href="https://www.google.com/chrome/" target="_blank">谷歌浏览器</a> 打开' +
        '	</div>' +
        '</div>';
    ...
    if (!vxgplayer.browserSupportsPluginPnacl()) {
      ...
      var html =
        ...
        '<div class="vxgplayer-unsupport">' +
        '	<div class="vxgplayer-unsupport-content">' +
        '	请点击<a href="/static/position/chrom-plugin.zip">下载VXG Player插件</a>' +
    ...
    window.vxgplayer.players[id] = new function(id, options) {
      ...
      if (typeof self.playerWidth === 'string') {
      ...
      var html =
        ...
        '	<div class="vxgplayer-volume-mute" style="display:none;"></div>' +
        '	<div class="vxgplayer-volume-down" style="display:none;"></div>' +
        '	<div class="vxgplayer-volume-progress vol7" style="display:none;"></div>' +
        '	<div class="vxgplayer-volume-up" style="display:none;"></div>' +
        '	<div class="vxgplayer-play"></div>' +
        '	<div class="vxgplayer-stop" style="display: none"></div>' +
        '	<div class="vxgplayer-fullscreen"></div>' +
        '</div>' +
        '<embed class="vxgplayer-module" autostart_parameter="' +
        autostart_parameter +
        '" name="nacl_module" id="' +
        id +
        '_nacl_module" path="' +
        ...
        '<div style="position: absolute;width: 100%;height: calc(100% - 35px);transform: translate(0, calc(-100% - 35px));"></div>';
      ...
      if (self.module.debug) console.log(id + ' -  init new player');
      ...
      if (self.player.hasAttribute('useragent-prefix')) {
        self.module.command(
          'setuseragent',
          self.player.getAttribute('useragent-prefix') + ' ' + navigator.userAgent
        );
      ...
      self.module.aspectRatio =
        self.player.hasAttribute('aspect-ratio') || self.player.hasAttribute('aspect-ratio-mode')
          ? 1
          : 0;
      ...
      el_volumeProgress.className = el_volumeProgress.className.replace(
        /vol\d+/g,
        'vol' + Math.ceil(self.module.volume * 10)
      );

      function moduleDidLoad() {
        if (self.module.debug) console.log('moduleDidLoad: ', arguments);
      ...
      function playerDidLoad() {
        if (self.module.debug) console.log('playerDidLoad: ', arguments);
      ...
      function connectToApp() {
        ...
        if (self.module.debug) console.log('connected port=' + self.module.port);
        var d = new Date();
        var n = d.getTime();
        self.module.port.name = '' + d.getTime();

        self.module.port.onDisconnect.addListener(function() {
          if (self.module.debug) console.log('disconnected port.name=' + self.module.port.name);
          self.module.command('stopnativeclient', '@' + self.module.port.name);

          self.module.port = undefined;
        });

        if (self.module.debug) console.log('connected port.name=' + self.module.port.name);
        ...
        self.module.port.onMessage.addListener(function(msg) {
          ...
          } else if (msg != undefined && msg.cmd == 'getversionapp') {
            if (0 == msg.data.indexOf('VERSION_APP')) {
              self.module.versionapp = msg.data.split(' ')[1];
              if (self.module.debug) console.log('=VERSION_APP ' + self.module.versionapp);
      ...
      self.reConnect = function() {
        if (self.hasError) {
          if (self.reconnect <= 0) return;
          self.reconnecting = setTimeout(function() {
            try {
              self.player.innerHTML === '' && !document.getElementById(self.player.id)
                ? null
                : self.play();
            } catch (error) {}
            if (self.reconnect <= 0 || !self.hasError) {
              return;
            }
            self.reconnect--;
          }, 20 * 1000);
      ...
      function handleMessage(msgEvent) {
        if (self.module.debug) console.log('Player ' + id + ': ' + msgEvent.data);
        if (msgEvent == undefined || msgEvent.data == undefined) return;
        if (msgEvent.data[0].charAt(0) == '@') {
          //proto native send to app
          if (self.module.port != undefined) self.module.port.postMessage(msgEvent.data);
        }
        ...
        // Player listener2: Skip picture: 170, frame_duration: 41, latencyms: 0<=frames_buf:0, b: 0, p: 0
        else if (0 == msgEvent.data.indexOf('VERSION_APP')) {
          ...
          if (self.module.handlerError) self.module.handlerError(self);
          ...
          if (self.onStateChangeCallback) self.onStateChangeCallback(self.module.vxgReadyState);
      ...
      self.play = function() {
        ...
        if (self.module.debug)
          console.log(
            'self.play self.module.url=' +
              self.module.url +
              ' self.module.is_opened=' +
              self.module.is_opened
          );
      ...
      self.mute = function() {
        ...
        if (self.module.mute) {
          ...
          el_volumeProgress.className = el_volumeProgress.className.replace(
            /vol\d+/g,
            'vol' + Math.floor(self.module.volume * 10)
          );
        }
